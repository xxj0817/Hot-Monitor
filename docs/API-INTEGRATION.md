...
- Bing：`https://www.bing.com/search?q=..&count=15&setlang=zh-hans&mkt=zh-CN&qft=+filterui:age-lt1440`，解析 `li.b_algo`，有则取 `news_dt` 作发布时间。
- 360资讯（新鲜度主力，带发布时间）：`https://news.so.com/ns?q=..&sort=1`，解析 `li.full-txt.res-list`（`data-url` 真实链接、`h3.g-title` 标题、`p.summary` 摘要、`span.time` 如“6小时前”/日期）。
- 360搜索：`https://www.so.com/s?q=..&rn=10`，解析 `li.res-list` 的 `data-mdurl` 真实链接。
- 百度(best-effort)：`https://www.baidu.com/s?wd=..&rn=10`，解析 `content_left` 的 `h3.c-title`；出现安全验证即自动降级。
